...
if (!function_exists('kleo_bp_search_form')):
    ...
    function kleo_bp_search_form ($show_profiles = false, $before_form = false)
    {
        ...
        {
            echo 'Please enable Buddypress and Extended profiles component for the search form!';
            return;
        }

    if (!isset($bp_search_fields['fields']) || count ((array)$bp_search_fields['fields']) == 0)
        {
             echo "<p>Please configure your form fields under Sweetdate -> Buddypress</p>";
             return;
        }
        ...
        <div class="form-wrapper">
            <div class="form-header">
                ...
                <p class="lead"><?php if($before_form) echo $before_form; else echo $bp_search_fields['before_form'];?></p>
                <?php endif; ?>
            </div>
            <!--Search form-->
        ...
        foreach ($bp_search_fields['fields'] as $sf)
        {
            ...
            switch ($field->type):
                ...
                case 'textbox':
                    ...
                    echo "<div class='seven mobile-four columns'><input type='text' name='$fname' id='$fname' value='$value' /></div>";
                break;
                ...
                case 'selectbox':
                    echo "<div class='five mobile-four columns'><label class='right inline' for='$fname'>".__($field->name,'kleo_framework')."</label></div>";
                    echo "<div class='seven mobile-four columns'><select class='expand' name='$fname' id='$fname'>";
                ...
                case 'multiselectbox':
                    echo "<div class='five mobile-four columns'><label class='right inline' for='$fname'>".__($field->name,'kleo_framework')."</label></div>";
                    echo "<div class='seven mobile-four columns'><select ".apply_filters('kleo_bp_search_multiselect_attributes',"")." class='expand' name='{$fname}[]' id='$fname'>";
                    echo "<option value=''></option>";
                    foreach ($options as $option)
                    {
                        $option->name = trim ($option->name);
                ...
                case 'radio':
                    ...
                    <?php
                    echo "</label></div>";
                    echo "<div class='seven mobile-four columns field_".$fname."'>";
                ...
                case 'checkbox':
                    ...
                    echo "<div class='seven mobile-four columns'>";
...
add_action ('kleo_search_form_horizontal', 'kleo_bp_search_form_horizontal');
            ...
            <form id="horizontal_search" action="<?php echo bp_get_root_domain (). '/'. bp_get_members_root_slug (). '/' ?>" method="get" class="<?php echo apply_filters('bp_search_horiz_extra_class','custom');?> dir-form twelve columns custom">
                <div class="row">
                    ...
                    <div class="two columns">
                        <label class="bp_search_form_filter"><?php echo $bp_search_fields['before_form_horizontal'];?></label>
                    </div>
                    <?php endif; ?>
                    ...
                    foreach ($bp_search_fields['fields_horizontal'] as $sf)
                    {
                        ...
                        switch ($field->type):
                            ...
                            case 'multiselectbox':
                                echo "<div class='two columns'><select ".apply_filters('kleo_bp_search_multiselect_attributes',"")." class='expand' name='{$fname}[]' id='$fname'>";
                                echo "<option value=''>".__( $field->name, 'kleo_framework')."</option>";
                                foreach ($options as $option)
                                {
                                    $option->name = trim ($option->name);
...
function kleo_bp_numrange ($name, $value)
{
	global $kleo_bp_textboxes;

	if (count ($kleo_bp_textboxes) > 1)
	{
		echo "<select name=\"$name\">\n";
		foreach ($kleo_bp_textboxes as $fid => $fname)
		{
			echo "<option value=\"$fid\"";
			if ($fid == $value)  echo " selected=\"selected\"";
			echo ">$fname &nbsp;</option>\n";
