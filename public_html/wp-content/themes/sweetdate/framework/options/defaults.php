    ...
    class Kleo_Options {
        ...
        */
        function __construct($sections = array(), $args = array(), $extra_tabs = array()) {    
            ...
            $defaults['menu_title'] = __('Options', 'kleo_framework');
            $defaults['page_icon'] = 'icon-themes';
            $defaults['page_title'] = __('Options', 'kleo_framework');
            ...
            $defaults['help_sidebar'] = __('', 'kleo_framework');
            ...
            if($this->args['page_type'] == 'submenu') {
                ...
                if(true === $this->args['allow_sub_menu']) {
                    ...
                    if(true === $this->args['show_import_export']) {
                        add_submenu_page(
                            $this->args['page_slug'],
                            __('Import / Export', 'kleo_framework'), 
                            __('Import / Export', 'kleo_framework'), 
                            $this->args['page_cap'], 
                    ...
                    if(true === $this->args['dev_mode']) {
                        add_submenu_page(
                            $this->args['page_slug'],
                            __('Dev Mode Info', 'kleo_framework'), 
                            __('Dev Mode Info', 'kleo_framework'), 
                            $this->args['page_cap'], 
        ...
        */
        function _enqueue() {
            ...
            wp_localize_script('squeen-opts-js', 'kleo_opts', array('reset_confirm' => __('Are you sure? You will lose all custom values.', 'kleo_framework'), 'opt_name' => $this->args['opt_name']));
             ...
             * 
             */
            $myfields = array('bg', 'button_set', 'color','upload', 'typography', 'checkbox','checkbox_hide_below','radio_img','radio_img_hide_bellow', 'select_hide_below');
        ...
        */
        function _options_page_html() {
            ...
            echo ' &nbsp; ';
            submit_button(__('Reset to Defaults', 'kleo_framework'), 'secondary', $this->args['opt_name'] . '[defaults]', false);
            ...
            if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('squeen-opts-saved') == '1') {
                if(isset($this->options['imported']) && $this->options['imported'] == 1) {
                    echo '<div id="squeen-opts-imported">' . apply_filters('squeen-opts-imported-text-' . $this->args['opt_name'], __('<strong>Settings Imported!</strong>', 'kleo_framework')) . '</div>';
                } else {
                    echo '<div id="squeen-opts-save">' . apply_filters('squeen-opts-saved-text-' . $this->args['opt_name'], __('<strong>Settings Saved!</strong>', 'kleo_framework')).'</div>';
                }
                delete_transient('squeen-opts-saved');
            }

            echo '<div id="squeen-opts-save-warn">' . apply_filters('squeen-opts-changed-text-' . $this->args['opt_name'], __('<strong>Settings have changed, you should save them!</strong>', 'kleo_framework')) . '</div>';
            echo '<div id="squeen-opts-field-errors">' . __('<strong><span></span> error(s) were found!</strong>', 'kleo_framework') . '</div>';
            echo '<div id="squeen-opts-field-warnings">' . __('<strong><span></span> warning(s) were found!</strong>', 'kleo_framework').'</div>';
            ...
            if(true === $this->args['show_import_export']) {
                ...
                echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="squeen-opts-group-tab-link-a" data-rel="import_export_default">' . $icon . ' <span>' . __('Import / Export', 'kleo_framework') . '</span></a>';
                echo '</li>';
            ...
            if(true === $this->args['dev_mode']) {
                ...
                echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="squeen-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default">' . $icon . ' <span>' . __('Dev Mode Info', 'kleo_framework') . '</span></a>';
                echo '</li>';
            }
            ...
            if(true === $this->args['show_import_export']) {
                ...
                echo '<h3>' . __('Import / Export Options', 'kleo_framework') . '</h3>';
                echo '<h4>' . __('Import Options', 'kleo_framework') . '</h4>';
                echo '<p><a href="javascript:void(0);" id="squeen-opts-import-code-button" class="button-secondary">' . __('Import from text', 'kleo_framework') . '</a> <a href="javascript:void(0);" id="squeen-opts-import-link-button" class="button-secondary">' . __('Import from URL', 'kleo_framework') . '</a></p>';

                echo '<div id="squeen-opts-import-code-wrapper">';

                echo '<div class="squeen-opts-section-desc">';
                echo '<p class="description" id="import-code-description">' . apply_filters('squeen-opts-import-file-description', __('Input your backup file below and hit Import to restore your sites options from a backup.', 'kleo_framework')) . '</p>';
                echo '</div>';
                ...
                echo '<div class="squeen-opts-section-desc">';
                echo '<p class="description" id="import-link-description">' . apply_filters('squeen-opts-import-link-description', __('Input the URL to another sites options set and hit Import to load the options from that site.', 'kleo_framework')) . '</p>';
                echo '</div>';
                ...
                echo '<p id="squeen-opts-import-action"><input type="submit" id="squeen-opts-import" name="' . $this->args['opt_name'] . '[import]" class="button-primary" value="' . __('Import', 'kleo_framework') . '"> <span>' . apply_filters('squeen-opts-import-warning', __('WARNING! This will overwrite any existing options, please proceed with caution!', 'kleo_framework')) . '</span></p>';
                echo '<div id="import_divide"></div>';

                echo '<h4>' . __('Export Options', 'kleo_framework') . '</h4>';
                echo '<div class="squeen-opts-section-desc">';
                echo '<p class="description">' . apply_filters('squeen-opts-backup-description', __('Here you can copy/download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).', 'kleo_framework')).'</p>';
                echo '</div>';

                echo '<p><a href="javascript:void(0);" id="squeen-opts-export-code-copy" class="button-secondary">' . __('Show export text', 'kleo_framework') . '</a>';
                //echo ' <a href="'.add_query_arg(array('feed' => 'kleoopts-'.$this->args['opt_name'], 'action' => 'download_options', 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), site_url()).'" id="squeen-opts-export-code-dl" class="button-primary">Download</a> <a href="javascript:void(0);" id="squeen-opts-export-link" class="button-secondary">' . __('Copy Link', 'kleo_framework') . '</a></p>';
                ...
                echo '</textarea>';
                echo '<input type="text" class="large-text" id="squeen-opts-export-link-value" value="' . add_query_arg(array('feed' => 'squeenopts-' . $this->args['opt_name'], 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), site_url()) . '" />';
            ...
            if(true === $this->args['dev_mode']) {
                ...
                echo '<h3>' . __('Dev Mode Info', 'kleo_framework') . '</h3>';
                echo '<div class="squeen-opts-section-desc">';
