...
function bp_album_install() {
	...
	update_site_option( 'bp-album-db-version', BP_ALBUM_DB_VERSION  );
        ...
        if(!get_site_option( 'bp_album_middle_size' ))
            update_site_option( 'bp_album_middle_size', 580);

        if(!get_site_option( 'bp_album_thumb_size' ))
            update_site_option( 'bp_album_thumb_size', 580);
...
function bp_album_setup_nav() {
	...
	$album_link = ( (isset($bp->displayed_user->id) && $bp->displayed_user->id) ? $bp->displayed_user->domain : $bp->loggedin_user->domain) . $bp->album->slug . '/';
	$album_link_title = (isset($bp->displayed_user->id) && $bp->displayed_user->id ) ? bp_word_or_name( __( "My pictures", 'bp-album' ), __( "%s's pictures", 'bp-album' ) ,false,false) : __( "My pictures", 'bp-album' );
...
function bp_album_privacy_level_permitted(){
	...
	elseif(is_super_admin())
		return 10;
	elseif ( isset($bp->displayed_user) && isset($bp->loggedin_user) && $bp->displayed_user->id && $bp->displayed_user->id == $bp->loggedin_user->id )
		return 6;
	elseif ( (isset($bp->displayed_user) && isset($bp->loggedin_user) && $bp->displayed_user->id && function_exists('friends_check_friendship') && friends_check_friendship($bp->displayed_user->id,$bp->loggedin_user->id) ) )
		return 4;
	else
		return 2;
}
...
function bp_album_limits_info(){
	...
	$owner_id = isset($pictures_template) && isset($pictures_template->picture) ? $pictures_template->picture->owner_id : $bp->loggedin_user->id;
	...
	foreach(array(0,2,4,6,10) as $i){
		...
		if( isset($pictures_template) && isset($pictures_template->picture) && $i==$pictures_template->picture->privacy )
...
function bp_album_record_activity($pic_data) {
	...
	$content = '<p> <a href="'. $primary_link .'" class="picture-activity-thumb" title="'.$title.'"><img alt="" src="'. $image_path .'" /></a>'.$desc.'</p>';
