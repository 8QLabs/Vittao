...
<form id="pmpro_form" class="pmpro_form custom" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
	...
	<thead>
		<tr>
			<th>
				<?php if(count($pmpro_levels) > 1) { ?><span class="pmpro_thead-msg"><a href="<?php echo pmpro_url("levels"); ?>" class="tiny radius button bordered"><?php _ex('change', 'change membership level link', 'pmpro');?></a></span><?php } ?><?php _e('Membership Level', 'pmpro');?>
			</th>						
		</tr>
	</thead>
	<tbody>                
		<tr>
			<td>				
				<p>
					<?php printf(__('You have selected the <strong class="label radius">%s</strong> membership level.', 'pmpro'), $pmpro_level->name);?>
				</p>
	...
	<?php if(!$skip_account_fields && !$pmpro_review) { ?>
        ...
        <?php } else { ?>
            <p><?php _e("Please Login or Register a new account in order to continue",'pmpro');?></p>
            <a href="#" data-reveal-id="login_panel" class="small secondary button radius"><i class="icon-user"></i> <?php _e("LOG IN", 'kleo_framework'); ?></a>
            <?php if(get_option('users_can_register')) { ?>
                <a href="<?php if (function_exists('bp_is_active')) bp_signup_page(); else echo get_bloginfo('url')."/wp-login.php?action=register"; ?>" class="small button radius"><i class="icon-group"></i> <?php _e("SIGN UP", 'kleo_framework'); ?></a>
            <?php } ?>
	...
	<?php } elseif($current_user->ID && !$pmpro_review) { ?>                        	                       										
		...
		<p>
			<?php printf(__('You are logged in as <strong>%s</strong>. If you would like to use a different account for this membership, <a href="%s" class="link">log out now</a>.', 'pmpro'), $current_user->user_login, wp_logout_url($_SERVER['REQUEST_URI'])); ?>			
		</p>
	<?php } ?>
	...
	<?php					
		...
		{						
		?>
		<table id="pmpro_tos_fields" class="pmpro_checkout top1em relative-columns" width="100%" cellpadding="0" cellspacing="0" border="0">
		<thead>
		<tr>
...
<?php echo wpautop($tospage->post_content)?>
	...
	<?php if( ((!function_exists('bp_is_active') && !is_user_logged_in()) || is_user_logged_in() ) && (empty($pmpro_stripe_lite) || $gateway != "stripe")) { ?>
	...
	<tbody>
		<tr>
			<td>
					...
					<div class="two columns"><label for="bfirstname" class="inline"><?php _e('First Name', 'pmpro');?></label></div>
					<div class="four columns"><input id="bfirstname" name="bfirstname" type="text" class="input <?php echo pmpro_getClassForField("bfirstname");?>" value="<?php echo esc_attr($bfirstname)?>" /></div>
			
				
					<div class="two columns"><label for="blastname" class="inline"><?php _e('Last Name', 'pmpro');?></label></div>
					<div class="four columns"><input id="blastname" name="blastname" type="text" class="input <?php echo pmpro_getClassForField("blastname");?>" size="30" value="<?php echo esc_attr($blastname)?>" /></div>
								
          <div class="clearfix"></div>
					<div class="two columns"><label for="baddress1" class="inline"><?php _e('Address 1', 'pmpro');?></label></div>
					<div class="four columns"><input id="baddress1" name="baddress1" type="text" class="input <?php echo pmpro_getClassForField("baddress1");?>" size="30" value="<?php echo esc_attr($baddress1)?>" /></div>
				
				
					<div class="two columns"><label for="baddress2" class="inline"><?php _e('Address 2', 'pmpro');?></label></div>
					<div class="four columns"><input id="baddress2" name="baddress2" type="text" class="input <?php echo pmpro_getClassForField("baddress2");?>" size="30" value="<?php echo esc_attr($baddress2)?>" /></div>
          <div class="clearfix"></div>
				...
				?>
					...
					<div class="two columns"><label for="bcity" class="inline"><?php _e('City', 'pmpro');?></label></div>
					<div class="four columns"><input id="bcity" name="bcity" type="text" class="input <?php echo pmpro_getClassForField("bcity");?>" size="30" value="<?php echo esc_attr($bcity)?>" /></div>
					
					
					<div class="two columns"><label for="bstate" class="inline"><?php _e('State', 'pmpro');?></label></div>															
					<div class="four columns"><input id="bstate" name="bstate" type="text" class="input <?php echo pmpro_getClassForField("bcity");?>" size="30" value="<?php echo esc_attr($bstate)?>" /></div>
					
					<div class="two columns"><label for="bzipcode" class="inline"><?php _e('Postal Code', 'pmpro');?></label></div>
					<div class="four columns"><input id="bzipcode" name="bzipcode" type="text" class="input <?php echo pmpro_getClassForField("bzipcode");?>" size="30" value="<?php echo esc_attr($bzipcode)?>" /></div>
				...
				<?php
					...
					<!--<div class="row">-->
						<div class="two columns"><label for="bcity_state_zip" class="inline"><?php _e('City, State Zip', 'pmpro');?></label></div>
						<div class="four columns"><input id="bcity" name="bcity" type="text" class="input <?php echo pmpro_getClassForField("bcity");?>" size="14" value="<?php echo esc_attr($bcity)?>" /></div> 
						<?php
              ...
              <div class="one columns">
							...
							{
							?>	
							<div class="three columns"><input id="bstate" name="bstate" type="text" class="input <?php echo pmpro_getClassForField("bstate");?>" size="2" value="<?php echo esc_attr($bstate)?>" /></div>
							<?php
							}
						?>
						<div class="three columns"><input id="bzipcode" name="bzipcode" type="text" class="input <?php echo pmpro_getClassForField("bzipcode");?>" size="5" value="<?php echo esc_attr($bzipcode)?>" /></div>
					<!--</div>-->
				...
				?>
          <div class="clearfix"></div>
					<div class="two columns"><label for="bphone"><?php _e('Phone', 'pmpro');?></label></div>
					<div class="four columns"><input id="bphone" name="bphone" type="text" class="input <?php echo pmpro_getClassForField("bphone");?>" size="30" value="<?php echo esc_attr($bphone)?>" /> 
				...
				?>
				<div class="clearfix">&nbsp;</div>
					<div class="two columns"><label for="bemail"><?php _e('E-mail Address', 'pmpro');?></label></div>
					<div class="four columns"><input id="bemail" name="bemail" type="text" class="input <?php echo pmpro_getClassForField("bemail");?>" size="30" value="<?php echo esc_attr($bemail)?>" /></div>
				...
				<?php
					...
					?>
						...
						<div class="two columns"><label for="bconfirmemail"><?php _e('Confirm E-mail', 'pmpro');?></label></div>
						<div class="four columns"><input id="bconfirmemail" name="bconfirmemail" type="text" class="input <?php echo pmpro_getClassForField("bconfirmemail");?>" size="30" value="<?php echo esc_attr($bconfirmemail)?>" /> </div>
	...
	?>
	<?php if((!function_exists('bp_is_active') && !is_user_logged_in()) || is_user_logged_in() ) { ?>
		...
		<tr valign="top">		
			<td>	
				...
				<?php if(empty($pmpro_stripe_lite) || $gateway != "stripe") { ?>
					...
					<div class="two columns"><label for="CardType"><?php _e('Card Type', 'pmpro');?></label></div>
					<div class="four columns"><select id="CardType" <?php if($gateway != "stripe") { ?>name="CardType"<?php } ?> class=" <?php echo pmpro_getClassForField("CardType");?>">
				...
				<?php } ?>
					...
					<div class="two columns"><label for="AccountNumber"><?php _e('Card Number', 'pmpro');?></label></div>
					<div class="four columns"><input id="AccountNumber" <?php if($gateway != "stripe" && $gateway != "braintree") { ?>name="AccountNumber"<?php } ?> class="input <?php echo pmpro_getClassForField("AccountNumber");?>" type="text" size="25" value="<?php echo esc_attr($AccountNumber)?>" <?php if($gateway == "braintree") { ?>data-encrypted-name="number"<?php } ?> /> </div>
				
			
				
					<div class="two columns"><label for="ExpirationMonth"><?php _e('Expiration Date', 'pmpro');?></label></div>
					<div class="two columns"><select id="ExpirationMonth" <?php if($gateway != "stripe") { ?>name="ExpirationMonth"<?php } ?> class=" <?php echo pmpro_getClassForField("ExpirationMonth");?>">
          ...
          <div class="two columns"><select id="ExpirationYear" <?php if($gateway != "stripe") { ?>name="ExpirationYear"<?php } ?> class=" <?php echo pmpro_getClassForField("ExpirationYear");?>">
						<?php
				...
				?>
					...
					<div class="two columns"><label for="CVV"><?php _ex('CVV', 'Credit card security code, CVV/CCV/CVV2', 'pmpro');?></label></div>
					<div class="three columns"><input class="input" id="CVV" <?php if($gateway != "stripe" && $gateway != "braintree") { ?>name="CVV"<?php } ?> type="text" size="4" value="<?php if(!empty($_REQUEST['CVV'])) { echo esc_attr($_REQUEST['CVV']); }?>" class=" <?php echo pmpro_getClassForField("CVV");?>" <?php if($gateway == "braintree") { ?>data-encrypted-name="cvv"<?php } ?> /></div>
          
          <small>(<a href="javascript:void(0);" onclick="javascript:window.open('<?php echo pmpro_https_filter(PMPRO_URL)?>/pages/popup-cvv.html','cvv','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=600, height=475');"><?php _ex("what's this?", 'link to CVV help', 'pmpro');?></a>)</small>
				...
				<?php if($pmpro_show_discount_code) { ?>
					...
					<div class="two columns"><label for="discount_code"><?php _e('Discount Code', 'pmpro');?></label></div>
					<div class="four columns"><input class="input <?php echo pmpro_getClassForField("discount_code");?>" id="discount_code" name="discount_code" type="text" size="20" value="<?php echo esc_attr($discount_code)?>" /></div>
          <div class="four columns"><input type="button" id="discount_code_button" name="discount_code_button" value="<?php _e('Apply', 'pmpro');?>" class="small radius button secondary" /></div>
          <br class="clear">
	...
	<div class="pmpro_submit">
		...
		<?php } else { ?>
			...
			<span id="pmpro_submit_span" <?php if(($gateway == "paypalexpress" || $gateway == "paypalstandard") && $pmpro_requirebilling) { ?>style="display: none;"<?php } ?>>
				<input type="hidden" name="submit-checkout" value="1" />		
				<div class="text-right"><input type="submit" class="small radius button" value="<?php if($pmpro_requirebilling) { _e('SUBMIT AND CHECK OUT', 'pmpro'); } else { _e('Submit and Confirm', 'pmpro');}?> &raquo;" />		</div>		
			</span>
		<?php } ?>
