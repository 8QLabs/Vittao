...
class buatp_admin_options {
    ...
    function buatp_set_option_arrays(){
        foreach((array)$this->all_type_info as $val){
            if($this->buatp_profile_data_setting['buatp_exclude_fields_for_'.$val['name']])
            $all_excludeds = (array)array_merge( (array)$all_excludeds , $this->buatp_profile_data_setting['buatp_exclude_fields_for_'.$val['name']]);
            $more_excluded_fields = buatp_get_all_fields("WHERE parent_id = 0 AND ID!=$this->buatp_type_field_id AND group_id > 1");
            if(count($more_excluded_fields))
                $all_excludeds = array_merge ((array)$all_excludeds,$more_excluded_fields);
    ...
    function buatp_basic_setting_page(){
        ...
        if($not_configured) {
            ...
            $page_list[0] = array(
                    'name' => 'buatp_page_selection',
                    'class' => 'buatp_page_selection',
                    'label' => __( sprintf('Select root page'), 'buatp' ),
                    'desc' => __( sprintf('<br>Select root page of user account type. This page will show nothing but rediect to deault user type directory.
                                   ...
                                   <br><b>%1$s/user-type/teachers</b>',  site_url()), 'buatp' ),
                    'type' => 'select',
            ...
            foreach((array)$this->all_type_info as $val) {
                $i++;
                $slug_list[$i] = array(
                    'name' => 'buatp_slug_selection_for_'.$val['name'],
                    'class' => 'buatp_slug_selection buatp_slug_selection_for_'.buatp_text_to_slug($val['name']),
                    'label' => __(  sprintf('Slug for <strong>%1$s</strong> type users',  $val['name']), 'buatp' ),
                    'type' => 'text',
                    'desc' => __( sprintf('URL: %1$s<br>Slug to show <strong>%2$s</strong> type users, no spacial characters, uppercase or spaces.',buatp_get_type_directory_url($val['name']), $val['name']), 'buatp' ),
                    'default' =>  buatp_get_type_directory_slug($val['name'])
                );
                $roles_list[$i] = array(
                    'name' => 'buatp_role_selection_for_'.$val['name'],
                    'class' => 'buatp_role_selection_for_'.buatp_text_to_slug($val['name']),
    ...
    function buatp_profile_data_setting_page(){
        ...
        if( count($this->buatp_basic_setting) < 4 ){
            $setting_fields = array(
                array(
                    ...
                    'label' => '',
                    'type' => 'html',
                    'value' => '<p class="buatp_notice">Please complete and save Basic Settings first to get more options</p>'
                )
            );
        ...
        foreach((array)$this->all_type_info as $val) {
              ...
              $groupss_list[$j] = array(
                    ...
                    'class' => 'buatp_exclude_groups_for_'.buatp_text_to_slug($val['name']),
              ...
              $fields_list[$j] = array(
                    ...
                    'class' => 'buatp_exclude_fields_for_'.buatp_text_to_slug($val['name']),
              ...
              $fields_list_reg[$j] = array(
                    ...
                    'class' => 'buatp_include_fields_at_registration_for_'.buatp_text_to_slug($val['name']),
                    ...
                    'type' => 'multicheck',
                    'options' => (array) $this->exclude_fields_array,
                    'before' => '<div class="checkbox_container">',
                    'after' => '</div>',  
                );
        ...
        $more_profile_data_fields = array(
                ...
                'desc' =>  __(sprintf('You may add some fields to show in members directory beside the member\'s avatar. Demo:<br>
                              <div id="buatp_members"><img class="demo_member" src="%1s" /></div>',  BUATP_IMG_URI.'loop-field-demo.png'), 'buatp' ),
                'type' => 'multicheck',
    ...
    function buatp_access_setting_page(){
        ...
        foreach((array)$this->all_type_info as $val) {
            $page_restriction[$i++] = array(
                ...
                'class' => 'buatp_restrict_page_for_'.buatp_text_to_slug($val['name']),
            ...
            $url_restriction[$i] = array(
                ...
                'class' => 'buatp_restrict_url_for_'.buatp_text_to_slug($val['name']),
            ...
            $redirect[$i] = array(
                    ...
                    'class' => 'buatp_restrict_redirect_for_'.buatp_text_to_slug($val['name']),
